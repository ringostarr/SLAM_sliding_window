"""
Forward-only SLAM launch: one clean pass through the slip patch (no turnaround). Use this for the slip summary plot; use slam.launch.py for the there-and-back mapping demo.: world + robot + bridge + panel mover + noise node
(pipeline) plus the scan matcher (info matrix + degeneracy) and the EKF.

    ros2 launch shifting_wall_slam_v2 slam_forward.launch.py render_engine:=ogre rviz:=true
"""
import os
...
def generate_launch_description():
    ...
    scan_match = Node(
        package="shifting_wall_slam_v2", executable="scan_match",
    ...
    ekf = Node(
        package="shifting_wall_slam_v2", executable="ekf_node",
        name="ekf_node", output="screen",
        parameters=common + [{"x0": 0.5, "y0": 0.9}],
    )

    mapping = Node(
        package="shifting_wall_slam_v2", executable="mapping",
        name="mapping_node", output="screen",
        parameters=common + [{"resolution": 0.1, "max_range": 8.0}],
    )

    logger = Node(
        package="shifting_wall_slam_v2", executable="logger",
        name="logger_node", output="screen",
        parameters=common,
    )

    autodrive = Node(
        package="shifting_wall_slam_v2", executable="autodrive",
        name="autodrive", output="screen",
        parameters=common + [{"speed": 0.5, "lane_y": 0.9, "stop_x": 19.0, "return_trip": False}],
